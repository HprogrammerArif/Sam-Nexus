...
const UploadMaterialForm = () => {
  ...
  const { mutateAsync, isPending } = useMutation({
    mutationFn: async (studyMaterials) => {
      const { data } = await axiosSecure.post(`/upload-materials`, studyMaterials);
      return data;
    },
    onSuccess: () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.table(studyMaterials);
      

      //Post request to server
      await mutateAsync(studyMaterials);
  ...
  const { _id, title, tutor_email } = job || {};
  console.log(job);
