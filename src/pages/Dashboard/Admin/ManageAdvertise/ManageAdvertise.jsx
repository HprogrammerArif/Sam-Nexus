...
import UpdateAdvertiseModal from "./UpdateAdvertiseModal";
...
const ManageAdvertise = () => {
  ...
  const { mutateAsync } = useMutation({
    mutationFn: async ({ id, status }) => {
      const { data } = await axiosSecure.patch(`/advertise/${id}`, { status });
      console.log(data);
      return data;
    },
    onSuccess: () => {
      ...
      // Kothin
      queryClient.invalidateQueries({ queryKey: ["all-advertise-data"] });
    },
  });
  ...
  return (
    <section className="container px-4 mx-auto">
      <div className="mb-4 lg:mb-8 flex justify-start items-center gap-3">
        ...
        <p className="text-2xl">Add Advertise Products</p>
        <button
